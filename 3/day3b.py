...
for y in range(len(engine)):
    for x in range(len(engine[y])):
        ...
        if current_part or next_char_is_digit:
            # As we read a part number we will check for adjacent gears
        ...
        if engine[y][x] not in "0123456789":
            ...
            # so we will always reset the current_part / valid_gears state
            # variables by the time we are done processing the line.
